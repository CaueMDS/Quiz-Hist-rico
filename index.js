...
function selecionarPerguntasAleatorias(base_de_dados){
    let questoesAleatorias = base_de_dados.sort(() => Math.random() - 0.5)
    return questoesAleatorias.slice(0, 10)
}

function exibePerguntas(arraySelecionado, index){
    console.log(`\n${index + 1}- ${arraySelecionado.pergunta}`);
    const respostaUsuario = entradaDados.question('Digite a resposta (ano): ');
    return respostaUsuario;
}

function confereResposta(resposta, base_de_dados){
    return resposta === base_de_dados.resposta
}

function finalizaQuiz(nome, nRespostas){
    console.log(`\n${nome} acertou ${nRespostas}/10\n`)
}
  
function iniciaQuiz() { 
    console.log('\n-------Quiz de Fatos Historico-------')
    let nomeJogador = entradaDados.question('\n\nDigite seu nome:')
    console.log('Responda as questões:')

    let arraySelecionado = selecionarPerguntasAleatorias(questoes)

    let quantRespostasCertas = 0
    arraySelecionado.forEach((pergunta, index) => {
        let respostaJogador = exibePerguntas(pergunta, index)
        let respostaCerta = confereResposta(respostaJogador, pergunta)
         
        if(respostaCerta){
            console.log('Resposta Correta!')
            quantRespostasCertas++
        }else{
            console.log('Resposta Incorreta!')
        }
    });

    finalizaQuiz(nomeJogador, quantRespostasCertas)
